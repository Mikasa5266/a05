...
export function useInterviewRecording(options = {}) {
  ...
  const stopAndUploadInterviewRecording = async () => {
    if (!interviewId?.value) return true;

    if (!interviewMediaRecorder) {
      recordingStatus.value =
        recordingStatus.value === "uploaded" ? "uploaded" : "failed";
      return true;
    }
    ...
    try {
      const res = await apiUploadInterviewRecording(
        interviewId.value,
        formData,
      );
  ...
  const cleanupInterviewRecording = () => {
    if (
      ...
      interviewMediaRecorder.state === "recording"
    ) {
      try {
        interviewMediaRecorder.stop();
      } catch {
