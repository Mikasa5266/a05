...
import { submitAnswer as apiSubmitAnswer, analyzeSpeechChunk as apiAnalyzeSpeechChunk } from '../api/interview'
...
export function useInterviewChat(options = {}) {
  ...
  const onAdvanceToNextQuestion = typeof options.onAdvanceToNextQuestion === 'function'
    ? options.onAdvanceToNextQuestion
    : () => {}
  const onCompleteInterview = typeof options.onCompleteInterview === 'function'
    ? options.onCompleteInterview
    : async () => {}
  const onScrollToBottom = typeof options.onScrollToBottom === 'function'
    ? options.onScrollToBottom
    : () => {}
  const onResetQuietSeconds = typeof options.onResetQuietSeconds === 'function'
  ...
  const getAdditionalStreams = () => [answerRecorderStream, analysisSourceStream].filter(Boolean)
