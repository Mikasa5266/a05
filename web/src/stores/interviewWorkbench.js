...
import { getLiveInterviewWorkbench, respondHumanInvitation } from '../api/interview'

const TAB_KEYS = ['invite_list', 'pending', 'processed', 'in_progress', 'history']
  ...
  actions: {
    ...
    async fetchWorkbench() {
      ...
      try {
        ...
        this.inviteList = Array.isArray(payload.invite_list) ? payload.invite_list : []
        this.pending = Array.isArray(payload.pending) ? payload.pending : []
        this.processed = Array.isArray(payload.processed) ? payload.processed : []
        this.inProgress = Array.isArray(payload.in_progress) ? payload.in_progress : []
