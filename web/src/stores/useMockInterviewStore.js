...
export const useMockInterviewStore = defineStore("mockInterviewStore", () => {
  ...
  const buildLocalHintDefaults = (questionText = "") => {
    const cleaned = String(questionText || "")
      .replace(/[？?]/g, "")
      .trim();
  ...
  const preloadShadowHintPack = async ({
    ...
    if (
      !shadowCoachEnabled.value ||
      !interviewId.value ||
      !currentQuestion.value
    )
      return;
    if (shadowHintPack.value.length >= 3 || shadowCoachHintPending.value)
      return;
  ...
  const maybeDispatchTieredHint = async ({
    ...
    for (let i = 0; i < milestones.length; i += 1) {
      if (
        quietSeconds.value >= milestones[i] &&
        !shadowHintDelivered.value[i]
      ) {
        const hint = shadowHintPack.value[i];
        showShadowBubble(hint);
        shadowCoachHints.value.unshift({
          text: hint,
          at: Date.now(),
          level: i + 1,
        });
  ...
  const startThinkingWatch = ({
    ...
    thinkingWatchTimer = setInterval(() => {
      ...
      if (
        isProcessing?.value ||
        latestAIMessage?.value?.type !== "question" ||
      ...
      if (
        typeof onMaybeDispatch === "function" &&
        quietChanged &&
        nextQuiet >= 20 &&
        !hintDispatchInFlight
      ) {
